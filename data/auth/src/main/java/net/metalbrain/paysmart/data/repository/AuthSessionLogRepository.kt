...
/**
 * Repository responsible for managing authentication session logs.
 *
 * This class handles the extraction of session-related claims from JWT ID tokens—specifically
 * the session ID (`sid`), session version (`sv`), and timestamp (`ts`)—and facilitates
 * their persistence into the local database via [AuthSessionLogDao].
 *
 * @property dao The Data Access Object used for performing database operations on session logs.
 */
