package net.metalbrain.paysmart.core.features.account.authentication.federated.screen
...
import net.metalbrain.paysmart.core.features.account.components.EmailVerificationBtn
...
fun FederatedLinkingScreen(
    isLoading: Boolean,
    clientId: String,
    onLinkGoogleCredential: (credential: AuthCredential, onError: (Throwable) -> Unit) -> Unit,
    onFacebookLogin: (activity: FragmentActivity, onError: (Throwable) -> Unit) -> Unit,
    onPasskeySetupClick: () -> Unit,
    onEmailVerifyClick: () -> Unit,
    onSkip: () -> Unit,
) {
    val activity = LocalActivity.current
    val launcher = rememberLauncherForActivityResult(ActivityResultContracts.StartActivityForResult()) {
        // No-op: result handled inside GoogleAuthProvider
    }
    ...
    Box(
        ...
        Column(
            ...
            AnimatedVisibility(
                visible = showContent.value,
                enter = fadeIn(animationSpec = tween(420, delayMillis = 80)) +
                    slideInVertically(initialOffsetY = { it / 6 }, animationSpec = tween(420, delayMillis = 80))
            ) {
                Surface(
                    ...
                    Column(
                        ...
                        GoogleSignInBtn(
                            ...
                            onCredentialReceived = { credential ->
                                onLinkGoogleCredential(credential) { e ->
                                    backendError.value = parseBackendError(e)
                                }
                            },
                            onError = { e -> backendError.value = parseBackendError(e) }
                        )
            ...
            AnimatedVisibility(
                visible = showContent.value,
                enter = fadeIn(animationSpec = tween(460, delayMillis = 160)) +
                    slideInVertically(initialOffsetY = { it / 8 }, animationSpec = tween(460, delayMillis = 160))
            ) {
                Surface(
                    modifier = Modifier.fillMaxWidth(),
