...
describe("identityUploadSessionHandler", () => {
    ...
    it("returns 401 when token is missing", async () => {
        ...
        await identityUploadSessionHandler(req, res);
    ...
    it("returns 503 when identity upload service is not configured", async () => {
        verifyIdToken.mockResolvedValue({ uid: "uid-1" });
        createSession.mockRejectedValue(new Error("IDENTITY_UPLOAD_KMS_KEY_NAME is not configured"));
        const req = {
            headers: {
                authorization: "Bearer token-1",
            },
...
//# sourceMappingURL=identityUploadSession.test.js.map