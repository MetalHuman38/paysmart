...
export function mountNotificationRoutes(app) {
    app.post("/notifications/installations/register", requireAppCheck, requireActiveSession, registerNotificationInstallationHandler);
    app.options("/notifications/installations/register", (_, res) => {
        corsify(res);
        res.status(204).end();
    });
    app.get("/notifications/preferences", requireAppCheck, requireActiveSession, getNotificationPreferencesHandler);
    app.options("/notifications/preferences", (_, res) => {
        corsify(res);
        res.status(204).end();
    });
    app.post("/notifications/preferences", requireAppCheck, requireActiveSession, updateNotificationPreferencesHandler);
