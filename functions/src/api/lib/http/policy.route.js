...
import { passkeySignInOptionsRateLimit, passkeySignInVerifyRateLimit, } from "../middleware/passkeySignInRateLimit.js";
export function mountAuthPolicyRoutes(app) {
    ...
    app.post("/auth/passkeys/signin/options", requireAppCheck, passkeySignInOptionsRateLimit, passkeySignInOptionsHandler);
    app.options("/auth/passkeys/signin/options", (_, res) => {
        corsify(res);
        res.status(204).end();
    });
    app.post("/auth/passkeys/signin/verify", requireAppCheck, passkeySignInVerifyRateLimit, passkeySignInVerifyHandler);
