...
export class GoogleVisionTextExtractionService {
    client;
    enabled;
    constructor(client = new ImageAnnotatorClient(), enabled = true) {
        this.client = client;
        this.enabled = enabled;
    }
    async extract(imageBytes, mimeType) {
        if (!this.enabled) {
...
export function extractCandidateFullNameFromVisionText(fullText) {
...
function extractLabeledName(lines) {
    for (let i = 0; i < lines.length; i += 1) {
        ...
        for (const keyword of CANDIDATE_NAME_KEYWORDS) {
            const directPattern = new RegExp(`^${escapeRegex(keyword)}\\s*[:\\-]?\\s+(.+)$`, "i");
...
function extractNameFromMrz(lines) {
...
function extractLikelyNameLine(lines) {
    const ranked = lines
...
//# sourceMappingURL=googleVisionTextExtractionService.js.map