...
import { decryptIdentityPayload, encryptIdentityPayload, } from "./identityPayloadCrypto.js";
describe("identityPayloadCrypto", () => {
    ...
    it("rejects payload when associated data does not match", () => {
        const key = randomBytes(32);
        const plainText = Buffer.from("sample-identity-binary-payload");
        const encrypted = encryptIdentityPayload(plainText, key, "uid:user-1|session:session-1|doc:passport");
        expect(() => decryptIdentityPayload(encrypted, key, "uid:user-1|session:session-1|doc:national_id")).toThrow();
    });
});
//# sourceMappingURL=identityPayloadCrypto.test.js.map