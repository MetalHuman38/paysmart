...
function drawHeader(canvas, invoice, topY) {
...
function drawPartyColumns(canvas, topY, invoice) {
...
function drawTotalsCard(canvas, topY, invoice) {
    const lines = [
    ...
    lines.forEach(([label, value], index) => {
        const font = index === lines.length - 1 ? "F2" : "F1";
...
function drawFooter(canvas, invoice, pageNumber, totalPages) {
    ...
    canvas.drawText(PAGE_MARGIN, marketingLineY, "Employers and agencies can discuss PaySmart invoicing rollout for their security teams.", {
        size: SMALL_FONT_SIZE,
        color: TEXT,
    });
...
class PdfPaginator {
    ...
    drawShiftGroups() {
        ...
        weekGroups.forEach((group, index) => {
            const requiredHeight = (needsSectionTitle ? measureShiftSectionTitleHeight() : 0) +
...
function buildPaymentLines(invoice) {
    return [
...
function measureSectionCardHeight(section) {
    const wrappedLines = section.lines
        .map((line) => wrapText(line, CONTENT_WIDTH - (CARD_PADDING * 2), BODY_FONT_SIZE))
        .flat();
    return (CARD_PADDING +
...
function measurePartyColumnsHeight(invoice) {
    const columnGap = 20;
    const innerWidth = CONTENT_WIDTH - (CARD_PADDING * 2);
    const columnWidth = (innerWidth - columnGap) / 2;
    const leftLines = [
...
function measureShiftSectionTitleHeight() {
    return 28;
}
function measureShiftWeekMatrixHeight() {
    return 24 + TABLE_HEADER_HEIGHT + (TABLE_ROW_HEIGHT * 3);
}
function measureTotalsCardHeight() {
    return 120;
}
function buildShiftWeekGroups(invoice) {
    const workedShifts = invoice.weekly.shifts.filter((shift) => (Number(shift.hoursInput) || 0) > 0);
...
function drawShiftWeekMatrix(canvas, topY, group, invoice) {
...
class PdfCanvas {
    ...
    drawTextRight(rightX, y, text, options = {}) {
        const size = options.size ?? BODY_FONT_SIZE;
    ...
    drawCircle(centerX, centerY, radius, options) {
        this.commands.push("q");
        if (options.fillColor) {
            this.commands.push(`${options.fillColor[0]} ${options.fillColor[1]} ${options.fillColor[2]} rg`);
        }
    ...
    drawRing(outerCenterX, outerCenterY, outerRadius, innerCenterX, innerCenterY, innerRadius, fillColor) {
        this.commands.push("q");
...
function drawPaySmartLogo(canvas, x, y, size) {
...
function measureTextWidth(text, fontSize, font = "F1") {
...
function resolveWeekEndingDate(workDate, fallback) {
...
function daySortValue(dayLabel) {
...
function buildPdf(pageStreams) {
    ...
    pageStreams.forEach((contentStream) => {
        ...
        objects.push(`<< /Type /Page /Parent 2 0 R /MediaBox [0 0 ${PAGE_WIDTH} ${PAGE_HEIGHT}] /Resources << /Font << /F1 ${pageStreams.length * 2 + 3} 0 R /F2 ${pageStreams.length * 2 + 4} 0 R >> >> /Contents ${contentObjectNumber} 0 R >>`);
...
//# sourceMappingURL=invoicePdfRenderer.js.map