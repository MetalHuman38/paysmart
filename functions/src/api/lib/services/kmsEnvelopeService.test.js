...
describe("KmsEnvelopeService", () => {
    it("fails fast for createEnvelopeKey when kms key is not configured", async () => {
        ...
        const service = new KmsEnvelopeService(tokenProvider, "", fetchLike);
    ...
    it("fails fast for decryptEnvelopeKey when kms key is not configured", async () => {
        ...
        const fetchLike = vi.fn();
        const service = new KmsEnvelopeService(tokenProvider, "", fetchLike);
        await expect(service.decryptEnvelopeKey("wrapped-key", "uid:u1|session:s1|doc:passport")).rejects.toThrow("IDENTITY_UPLOAD_KMS_KEY_NAME is not configured");
        expect(tokenProvider.getAccessToken).not.toHaveBeenCalled();
        expect(fetchLike).not.toHaveBeenCalled();
    });
});
//# sourceMappingURL=kmsEnvelopeService.test.js.map