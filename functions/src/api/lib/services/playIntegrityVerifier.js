...
export class PlayIntegrityVerifier {
    ...
    options;
    fetchLike;
    constructor(tokenProvider, options, fetchLike = fetch) {
        this.tokenProvider = tokenProvider;
        this.options = options;
        this.fetchLike = fetchLike;
    }
    async verify(attestationToken, expectedNonce) {
        ...
        const response = await this.fetchLike(`${PLAY_INTEGRITY_ENDPOINT}/${this.options.packageName}:decodeIntegrityToken`, {
            method: "POST",
            headers: {
        ...
        if (!response.ok) {
            ...
            throw new Error(`Play Integrity decode failed (${response.status}): ${errorBody}`);
        }
    ...
    validatePayload(payload, expectedNonce, rawToken) {
        ...
        const appRecognitionVerdictRaw = payload.appIntegrity?.appRecognitionVerdict ?? "";
        ...
        const deviceRecognitionVerdicts = (payload.deviceIntegrity?.deviceRecognitionVerdict ?? [])
            ...
            .filter(Boolean);
        const hasAllowedDeviceVerdict = deviceRecognitionVerdicts.some((value) => this.options.allowedDeviceVerdicts.has(value));
        if (!hasAllowedDeviceVerdict) {
            throw new Error("Play Integrity device verdict is not allowed");
        }
        const appLicensingVerdictRaw = payload.accountDetails?.appLicensingVerdict ?? "";
    ...
    verifyFallbackToken(token, expectedNonce) {
...
//# sourceMappingURL=playIntegrityVerifier.js.map