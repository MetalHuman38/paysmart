...
export const processProductUpdateCampaign = onDocumentWritten({
    ...
    const service = new ProductUpdateCampaignService(new FirestoreProductUpdateCampaignRepository(firestore), new FirestoreNotificationPreferencesRepository(firestore), notificationDelivery, config.getMailer(), new EmailSubscriptionService(config.getEmailUnsubscribeSecret(), new URL("/email/unsubscribe", config.getPublicSiteOrigin()).toString()), new FirestoreUserRepository(firestore), auth, config.getProductUpdatesUrl());
    await service.processReadyCampaign(event.params.campaignId);
});
//# sourceMappingURL=processProductUpdateCampaign.js.map