import { AddressLookupResult } from "../types/address.js";
import {
  GoogleAddressValidationLookupInput,
...
import {
  classifyAddressValidationVerdict,
  toMapPinBounds,
} from "../result.js";

export async function googleAddressValidationLookup(
  input: GoogleAddressValidationLookupInput
  ...
  const payload = await postJson<GoogleAddressValidationResponse>(
    url,
    {
      address: {
  ...
  const resolvedCountryCode =
    normalizeOptionalText(postalAddress?.regionCode)?.toUpperCase() ?? countryCode;
  const resolvedPostCode = normalizeOptionalText(postalAddress?.postalCode) ?? postcode;
  const line1 =
    normalizeOptionalText(postalAddress?.addressLines?.[0]) ??
