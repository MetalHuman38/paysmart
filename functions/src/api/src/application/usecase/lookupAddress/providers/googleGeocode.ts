...
import { GoogleGeocodeResponse } from "../types/addressLookupProvider.js";
...
export async function googleGeocode(
  ...
  const payload = await fetchJson<GoogleGeocodeResponse>(url, {}, 8_000).catch(() => null);
  ...
  for (const result of payload.results) {
    ...
    const city =
      normalizeOptionalText(components.get("locality")) ??
      normalizeOptionalText(components.get("postal_town"));
    const stateOrRegion = normalizeOptionalText(
