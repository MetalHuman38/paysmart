...
import {
  GoogleAddressValidationBounds,
  GoogleAddressValidationVerdict,
} from "./types/addressLookupProvider.js";
import {
  buildFullAddressWithHouse,
...
export function classifyAddressValidationVerdict(input: {
...
}): { decision: AddressDecision; reasons: string[] } {
  ...
  const countryMatches = isCountryMatch(
    input.requestedCountryCode,
    input.resolvedCountryCode
  );
  if (!countryMatches) {
    reasons.push("country_mismatch");
  }

  const postCodeMatches = isPostcodeMatch(
    input.requestedPostCode,
    input.resolvedPostCode
  );
  if (!postCodeMatches) {
    reasons.push("postcode_mismatch");
  }

  const hasPremiseLevelGranularity =
    hasPremiseGranularity(verdict?.validationGranularity) &&
...
export function toMapPinBounds(
  bounds?: GoogleAddressValidationBounds
): MapPinPayload["bounds"] | undefined {
...
function hasPremiseGranularity(granularity?: string): boolean {
  const normalized = normalizeOptionalText(granularity)?.toUpperCase();
  return normalized === "PREMISE" || normalized === "SUB_PREMISE";
}

function isCountryMatch(requestedCountryCode: string, resolvedCountryCode: string): boolean {
...
function isPostcodeMatch(requestedPostCode: string, resolvedPostCode: string): boolean {
