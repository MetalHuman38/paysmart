...
function parseLimit(raw: unknown): number {
  ...
  if (bounded <= 0) return DEFAULT_LIMIT;
...
function timestampToMillis(value: unknown): number {
  ...
  if (typeof maybeToMillis === "function") {
    try {
      const millis = maybeToMillis.call(value) as unknown;
...
function latestTimestamp(...values: unknown[]): number {
  return values.reduce<number>((max, next) => {
    const current = timestampToMillis(next);
    return current > max ? current : max;
  }, 0);
}

async function queryLatestDocs(
  collectionRef: any,
  limit: number,
  preferredOrderField = "updatedAt"
) {
  try {
...
export async function adminUserActivityFeedHandler(req: Request, res: Response) {
  try {
    const uid = normalizedUid(req.params.uid);
    if (!uid) {
      return res.status(400).json({ error: "Missing uid" });
    ...
    const [sessionStateSnap, identityUploadsSnap, providerSessionsSnap, addMoneySnap, walletTxSnap] =
      await Promise.all([
        ...
        queryLatestDocs(
          userRef.collection("payments").doc("add_money").collection("sessions"),
          perSourceLimit
        ),
    ...
    sessionStateSnap.docs.forEach((doc: any) => {
    ...
    identityUploadsSnap.docs.forEach((doc: any) => {
      const data = asRecord(doc.data());
    ...
    providerSessionsSnap.docs.forEach((doc: any) => {
      const data = asRecord(doc.data());
      const status = asString(data.status) || "unknown";
    ...
    addMoneySnap.docs.forEach((doc: any) => {
      const data = asRecord(doc.data());
      const status = asString(data.status) || "unknown";
