...
import { IdentityDocumentType } from "../domain/model/identityUpload.js";

const SUPPORTED_DOCUMENT_TYPES: ReadonlySet<IdentityDocumentType> = new Set([
  "passport",
  "drivers_license",
  "national_id",
]);

export async function identityUploadSessionHandler(req: Request, res: Response) {
  try {
    ...
    const documentType = (req.body?.documentType ?? "").toString().trim() as IdentityDocumentType;
    ...
    if (message.includes("STORAGE_EMULATOR_SIGNED_URL_UNSUPPORTED")) {
      return res.status(503).json({
        error:
          "Storage emulator does not support signed URL uploads for identity. Run without Storage emulator for identity E2E, or add a local server-upload fallback.",
