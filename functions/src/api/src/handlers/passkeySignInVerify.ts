...
export async function passkeySignInVerifyHandler(req: Request, res: Response) {
  ...
  try {
    ...
    if (
      message.includes("PASSKEY_CHALLENGE") ||
...
async function resolveActiveSessionClaims(uid: string): Promise<{ sid: string; sv: number }> {
...
function extractCredentialIdForAudit(input: unknown): string {
  if (!input) return "";
  ...
  if (typeof input === "string") {
    ...
    try {
      ...
      return (
        (parsed.id ?? parsed.rawId ?? "").toString().trim()
      );
    } catch {
      return "";
    }
...
function toCredentialIdPreview(value: string): string | undefined {
