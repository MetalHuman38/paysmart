...
export class FirestoreAddMoneyRepository implements AddMoneyRepository {
  constructor(
    private readonly firestore: Firestore,
    private readonly stripe: StripePaymentsService,
    private readonly managedCards: ManagedCardRepository,
  ...
  async getSessionStatus(
    ...
    const current = snap.data() as AddMoneySessionDoc;
    if (current.uid !== uid) {
      throw new Error("Add money session not found");
    }
    ...
    const stripeSession =
      isPaymentIntentSession ? null : await this.stripe.retrieveCheckoutSession(cleanSessionId);
    const stripeIntent =
      isPaymentIntentSession ? await this.stripe.retrievePaymentIntent(cleanSessionId) : null;
    const status = stripeIntent ?
      this.derivePaymentIntentStatus(stripeIntent.status) :
      this.deriveCheckoutStatus(stripeSession as StripeCheckoutSession);
    ...
    const stripePaymentStatus =
      stripeIntent?.status ?? stripeSession?.paymentStatus ?? current.stripePaymentStatus;
  ...
  async applyWebhook(
    ...
    const stripeSession = await this.stripe.retrieveCheckoutSession(
      asString(event.dataObject.id)
    );
    ...
    if (status === "succeeded") {
      await this.applySettlementIfNeeded(
        uid,
        stripeSession.id,
        stripeSession.amountTotalMinor,
  ...
  private async applySettlementIfNeeded(
    ...
    return this.firestore.runTransaction(async (tx) => {
      ...
      const sessionData = sessionSnap.data() as AddMoneySessionDoc;
  ...
  private deriveCheckoutStatus(session: StripeCheckoutSession): AddMoneySessionStatus {
  ...
  private derivePaymentIntentStatus(statusRaw: string): AddMoneySessionStatus {
