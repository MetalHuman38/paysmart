...
const ALLOWED_DOCUMENT_TYPES: ReadonlySet<IdentityDocumentType> = new Set([
  "passport",
  "drivers_license",
  "national_id",
]);
...
export class FirestoreIdentityUploadRepository
  implements IdentityUploadRepository
{
  constructor(
    private readonly firestore: Firestore,
    private readonly storageBucketName: string,
    private readonly attestationVerifier: PlayIntegrityVerifier,
    private readonly kmsEnvelopeService: KmsEnvelopeService,
  ...
  private ensureValidInput(input: CreateIdentityUploadSessionInput) {
  ...
  async createSession(
    ...
    const sessionDoc: IdentityUploadSessionDoc = {
      sessionId,
      uid,
  ...
  async uploadEncryptedPayload(
    ...
    if (!storageEmulatorHost) {
      throw new Error(
        "EMULATOR_ONLY: direct payload upload endpoint is only available when FIREBASE_STORAGE_EMULATOR_HOST is configured"
      );
    }
  ...
  async commitSession(
    ...
    const payloadSha256 = input.payloadSha256.trim();
    if (!payloadSha256) throw new Error("Missing payloadSha256");

    const attestationJwt = input.attestationJwt.trim();
    if (!attestationJwt) throw new Error("Missing attestationJwt");
    ...
    await ref.set(
      {
        ...
        attestationDeviceRecognitionVerdicts:
          attestation.deviceRecognitionVerdicts,
