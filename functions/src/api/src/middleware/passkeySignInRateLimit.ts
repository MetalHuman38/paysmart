...
export const passkeySignInOptionsRateLimit = buildLimiter(
  Math.max(8, Math.floor(baseAuthMax))
);

export const passkeySignInVerifyRateLimit = buildLimiter(
  Math.max(5, Math.floor(baseAuthMax / 2))
);
