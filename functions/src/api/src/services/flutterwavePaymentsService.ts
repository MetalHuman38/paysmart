...
export interface CreateFlutterwaveTopupSessionInput {
  uid: string;
  amountMinor: number;
  currency: string;
...
export interface FlutterwavePaymentsServiceOptions {
  secretKey: string;
  ...
  clientId: string;
  clientSecret: string;
  virtualAccountExpirySeconds: number;
}
...
export class FlutterwavePaymentsService {
  ...
  constructor(private readonly options: FlutterwavePaymentsServiceOptions) {}
  ...
  async createTopupSession(
    ...
    const checkoutUrl =
      asString(response.checkout_url) || asString(response.payment_url) || undefined;
    const status = asString(response.status) || "pending";
    const createdAtMs = parseDateToMs(asString(response.created_datetime)) ?? Date.now();
    const expiresAtMs =
      parseDateToMs(asString(response.account_expiration_datetime)) ??
      createdAtMs + this.options.virtualAccountExpirySeconds * 1000;
    const virtualAccount = this.mapVirtualAccountDetails(
      response,
      input.reference,
      buildVirtualAccountNarration(input.customer)
    );
  ...
  async retrieveTransactionStatus(
    ...
    const data = await this.providerRequest(
      `/charges/${encodeURIComponent(cleanId)}`,
      "GET"
    );
    const response = asRecord(data.data);
  ...
  async retrieveLatestVirtualAccountCharge(
    ...
    const data = await this.providerRequest(
      `/charges?virtual_account_id=${encodeURIComponent(cleanVirtualAccountId)}`,
      "GET"
    );
  ...
  parseWebhookEvent(
    ...
    if (this.options.webhookSecretHash.trim()) {
      ...
      if (normalizedName === "flutterwave-signature") {
        if (
          !verifyFlutterwaveHmac(
            rawPayload,
            trimmedSignature,
            this.options.webhookSecretHash
          )
        ) {
      ...
      throw new Error(
        "FLUTTERWAVE_WEBHOOK_SECRET_HASH is required when unsigned webhooks are disabled"
      );
    }
  ...
  private hasOauthCredentials(): boolean {
    return (
      this.options.clientId.trim().length > 0 &&
  ...
  private mapPermanentFundingAccount(
    ...
    const bankName =
      asString(response.account_bank_name) ||
  ...
  private mapVirtualAccountDetails(
    response: Record<string, unknown>,
    fallbackReference: string,
    fallbackAccountName: string
  ): FlutterwaveVirtualAccountDetails {
    const bank = asRecord(response.bank);
    const accountNumber =
      asString(response.account_number) || asString(response.accountNumber);
    const bankName =
  ...
  private async resolveAuthorizationHeader(): Promise<string> {
    ...
    const expiresInSeconds =
      asNumber(parsed.expires_in) || asNumber(asRecord(parsed.data).expires_in) || 600;
  ...
  private mapTransaction(data: Record<string, unknown>): FlutterwaveTransactionStatus {
    const transactionId =
      asString(data.id) || asString(data.flw_ref) || asString(data.transaction_id);
...
function verifyFlutterwaveHmac(
  ...
  const expectedBase64 = createHmac("sha256", secretHash.trim())
    ...
    .digest("base64");
  return (
    safeCompare(signatureHeader, expectedHex) ||
