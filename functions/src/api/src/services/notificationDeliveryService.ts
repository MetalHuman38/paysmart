import type { Messaging, MulticastMessage, SendResponse } from "firebase-admin/messaging";
import {
  FirestoreNotificationRepository,
  type NotificationInboxWrite,
  type NotificationPushInstallation,
...
export type TransactionalNotificationRequest = NotificationInboxWrite;
...
export class NotificationDeliveryService {
  constructor(
    private readonly notifications: FirestoreNotificationRepository,
    private readonly messaging: Messaging
  ) {}

  async deliverToUser(
    uid: string,
    input: TransactionalNotificationRequest,
    ...
    const installations = dedupeInstallations(
      await this.notifications.listPushInstallations(uid)
    );
    ...
    for (const chunk of chunkInstallations(installations, MAX_MULTICAST_TOKENS)) {
      const response = await this.messaging.sendEachForMulticast(
        buildMulticastMessage(chunk, notificationId, input)
      );
...
function buildMulticastMessage(
  installations: NotificationPushInstallation[],
  notificationId: string,
  input: TransactionalNotificationRequest
): MulticastMessage {
...
function dedupeInstallations(
  ...
  const unique: NotificationPushInstallation[] = [];
...
function chunkInstallations(
  installations: NotificationPushInstallation[],
  size: number
): NotificationPushInstallation[][] {
  const chunks: NotificationPushInstallation[][] = [];
...
function isInvalidTokenResponse(response: SendResponse): boolean {
...
function resolveAndroidChannelId(channel: string): string {
  switch (channel) {
