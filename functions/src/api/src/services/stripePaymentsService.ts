...
export interface CreateTopupCheckoutSessionInput {
  uid: string;
  amountMinor: number;
  currency: string;
  successUrl: string;
...
export interface CreateTopupPaymentIntentInput {
  uid: string;
  amountMinor: number;
  currency: string;
  idempotencyKey?: string;
...
export interface StripeCustomerCardPaymentMethod {
  id: string;
  brand: string;
  last4: string;
  expMonth: number;
...
export interface StripePaymentsServiceOptions {
  secretKey: string;
...
export class StripePaymentsService {
  private static readonly EPHEMERAL_KEY_API_VERSION = "2020-03-02";

  constructor(private readonly options: StripePaymentsServiceOptions) {}
  ...
  ensureConfigured() {
    ...
    if (this.options.secretKey.startsWith("pk_")) {
      throw new Error(
        "STRIPE_SECRET_KEY must be a secret key (sk_ or rk_), not a publishable key (pk_)"
      );
    }
  }
  ...
  async createTopupPaymentIntent(
    input: CreateTopupPaymentIntentInput
  ...
  async retrieveCheckoutSession(sessionId: string): Promise<StripeCheckoutSession> {
    ...
    const data = await this.stripeRequest(
      `/checkout/sessions/${escaped}?expand%5B%5D=payment_intent`,
      "GET"
    );
    return this.mapCheckoutSession(data);
  }
  ...
  async retrievePaymentIntent(paymentIntentId: string): Promise<StripePaymentIntent> {
    ...
    const data = await this.stripeRequest(
      `/payment_intents/${escaped}?expand%5B%5D=payment_method`,
      "GET"
    );
    return this.mapPaymentIntent(data);
  }
  ...
  async createEphemeralKey(customerId: string): Promise<StripeEphemeralKey> {
    this.ensureConfigured();
    const payload = new URLSearchParams();
    ...
    const data = await this.stripeRequest(
      ...
      "POST",
      payload,
      { "Stripe-Version": StripePaymentsService.EPHEMERAL_KEY_API_VERSION }
    );
  ...
  async listCustomerCardPaymentMethods(
    customerId: string
  ): Promise<StripeCustomerCardPaymentMethod[]> {
    this.ensureConfigured();
    const escaped = encodeURIComponent(customerId.trim());
    const data = await this.stripeRequest(
      `/payment_methods?customer=${escaped}&type=card`,
      "GET"
    );
  ...
  parseWebhookEvent(rawPayload: string, signatureHeader?: string): StripeWebhookEvent {
    this.ensureConfigured();
    ...
    if (this.options.webhookSigningSecret) {
      ...
      if (
        !verifyStripeSignature(rawPayload, signatureHeader, this.options.webhookSigningSecret)
      ) {
  ...
  private mapCheckoutSession(data: Record<string, unknown>): StripeCheckoutSession {
  ...
  private mapPaymentIntent(data: Record<string, unknown>): StripePaymentIntent {
    const id = asString(data.id);
  ...
  private mapCustomer(data: Record<string, unknown>): StripeCustomer {
    const id = asString(data.id);
    if (!id) {
    ...
    return {
      ...
      defaultPaymentMethodId:
        asString(invoiceSettings.default_payment_method) || undefined,
    };
  }

  private mapCustomerCardPaymentMethod(
...
function resolvePaymentIntentId(raw: unknown): string | undefined {
