...
export function renderEmailTemplate(message: MailMessage): RenderedEmailTemplate {
  switch (message.kind) {
...
function renderVerificationEmail(
  ...
  return renderBaseEmail({
    ...
    footer:
      "<p style=\"margin:0;\">This link opens the PaySmart verification handoff and works best on your Android device.</p>",
  });
}

function renderProductUpdateEmail(
  message: ProductUpdateEmailInput
): RenderedEmailTemplate {
...
function renderSecurityAlertEmail(
  message: SecurityAlertEmailInput
): RenderedEmailTemplate {
  return renderBaseEmail({
    subject: message.subject?.trim() || message.title.trim(),
    ...
    ctaUrl: message.ctaUrl,
    footer:
      "<p style=\"margin:0;\">If this was not you, review your PaySmart security settings immediately.</p>",
  });
}

function renderTransactionalStatusEmail(
  message: TransactionalStatusEmailInput
): RenderedEmailTemplate {
  return renderBaseEmail({
    subject: message.subject?.trim() || message.title.trim(),
    ...
    ctaUrl: message.ctaUrl,
    footer:
      "<p style=\"margin:0;\">This message relates to your PaySmart account activity.</p>",
  });
}
...
function renderBaseEmail(input: BaseEmailInput): RenderedEmailTemplate {
  const button =
...
function renderPlainTextEmail(input: BaseEmailInput): string {
  const parts = [
    input.eyebrow,
    "",
    input.title,
