...
describe("BeforeSignInPolicyUsecase", () => {
    beforeEach(() => {
        vi.clearAllMocks();
        authSessionRepo.recordSignInSession.mockResolvedValue({
            sid: "sid-test",
            sv: 1,
        });
    });
    it("allows linked federated login and syncs baseline providers without consuming link grant", async () => {
        securityRepo.createIfMissing.mockResolvedValue(makeSecurity({
        ...
        securityRepo.persistProviders.mockResolvedValue(undefined);
        auditLogRepo.log.mockResolvedValue(undefined);
        const usecase = new BeforeSignInPolicyUsecase(securityRepo, auditLogRepo, authService, authSessionRepo);
        const event = {
            data: {
                uid: "uid-1",
                phoneNumber: null,
        ...
        expect(securityRepo.persistProviders).toHaveBeenCalledWith("uid-1", expect.arrayContaining(["phone", "google.com"]), { consumeLinkingGrant: false });
        expect(auditLogRepo.log).not.toHaveBeenCalledWith("uid-1", "federated_link_confirmed", expect.anything());
    });
    it("denies federated sign-in when both event phone and auth phone are missing", async () => {
        ...
        await expect(usecase.execute(event)).rejects.toThrow("Federated login requires verified phone number");
    ...
    it("denies new federated provider when linking flag is not enabled and baseline exists", async () => {
        ...
        await expect(usecase.execute(event)).rejects.toThrow("Federated account linking not authorized");
    ...
    it("allows new federated provider when linking flag is enabled and consumes grant", async () => {
        ...
        expect(auditLogRepo.log).toHaveBeenCalledWith("uid-4", "federated_link_confirmed", { providerIds: expect.arrayContaining(["phone", "google.com"]) });
    });
    it("does not block sign-in when session logging fails", async () => {
        ...
        authSessionRepo.recordSignInSession.mockRejectedValue(new Error("write failed"));
        const usecase = new BeforeSignInPolicyUsecase(securityRepo, auditLogRepo, authService, authSessionRepo);
        const event = {
...
//# sourceMappingURL=BeforeSignInPolicyUsecase.test.js.map