...
import {
  deleteUserData,
  getUserDoc,
  lookupUidByAppScopedId,
  writeDeletionLog,
} from "./services/facebookDeletionHelpers.js";
import { FacebookSignedRequestPayload, parseSignedRequest } from "./facebookSignedRequest.js";
...
function readSignedRequest(req: Request): string | null {
  ...
  if (contentType.includes("application/x-www-form-urlencoded")) {
    const body = (req.body as any)?.signed_request;
    return typeof body === "string" ? body : null;
  }

  if (typeof (req.body as any)?.signed_request === "string") {
    return (req.body as any).signed_request;
  }

  return null;
}

export async function facebookDataDeletionHandler(
  req: Request,
  res: Response
) {
